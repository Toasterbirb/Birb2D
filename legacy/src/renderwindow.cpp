...
bool isTextEntity(Birb2D::Entity entity)
{
...
void Birb2D::RenderWindow::render(Entity& p_entity)
{
	...
	/* Handle normal entities and text entities differently */
	if (isTextEntity(p_entity))
	{
		src.x = p_entity.text.frame.x;
		src.y = p_entity.text.frame.y;
		src.w = p_entity.text.frame.w;
		src.h = p_entity.text.frame.h;

		dst.x = p_entity.text.frame.x;
		dst.y = p_entity.text.frame.y;
		dst.w = p_entity.text.frame.w * p_entity.localScale;
		dst.h = p_entity.text.frame.h * p_entity.localScale;

		centerPoint = Vector2f(p_entity.text.frame.w / 2, p_entity.text.frame.h / 2);
	}
	else
	{
		...
		centerPoint = Vector2f(p_entity.rect.w / 2, p_entity.rect.h / 2);
	}
	...
	{
		/* Text entity */
		if (SDL_RenderCopyEx(renderer, p_entity.text.texture.sdlTexture, &src, &dst, p_entity.angle, &center, SDL_FLIP_NONE) < 0)
		{
			Debug::Log("Error during text rendering: " + std::string(SDL_GetError()) + ". Entity: [" + p_entity.name + "]. Texture dimensions: " + p_entity.text.texture.dimensions.print(), Debug::error);
		}
	}
	else
	{
