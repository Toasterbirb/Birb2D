...
namespace Birb2D
{
	...
	void Window::InitSDL_ttf()
	{
		...
		else
		{
			...
			Debug::Log("SDL_ttf Initialized successfully");
		}
	}
	...
	void Window::InitSDL_image()
	{
		...
		else
		{
			...
			Debug::Log("SDL_image Initialized successfully");
		}
	}
	...
	SDL_Texture* Render::LoadTexture(std::string p_filePath)
	{
	...
	TTF_Font* Render::LoadFont(std::string p_filePath, int p_fontSize)
	{
		TTF_Font* font;
